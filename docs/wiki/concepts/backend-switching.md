...
updated: 2026-04-23
graph_node_id: concept_backend_switching
---
...
Inference parameters differ per model: the uncensored model needs different temperature/top_p than the obliterated one. Baking them into the backend profile (rather than having a single global config) lets each model run with its optimal settings. See `docs/raw/plan-llmBackendSwitching.prompt.md` for the original design spec.
